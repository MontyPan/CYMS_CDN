...
window.onload = function () {
    hid = getValue("hid");
    addItem = getValue("addItem");
    xItem = getValue("xItem");
}
// 初始給值 end
...
// 取得 可预约 約滿 之日期 start
$.ajax({
    url: "http:///jkgl.cymsimc.com/cyms/return_health_check_reservation_date.php",
    type: "GET",
    cache: false,
    async: false,
    dataType: "json",
...
function getDetailByDate(selectedDate) {
    window.location.href = 'healthCheck4.html?selectedDate=' + selectedDate + '&hid=' + hid + '&addItem=' + addItem+ '&xItem=' + xItem;
}
//製作日曆 end